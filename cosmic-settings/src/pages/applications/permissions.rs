...
/// ScreenCast, RemoteDesktop, and Device portals ("remember this selection").
const TABLES: &[&str] = &["screencast", "remote-desktop", "devices"];

/// Device ids within the `devices` table, each shown as its own category.
const DEVICES: &[&str] = &["camera", "microphone"];
...
pub struct Grant {
    ...
    id: String,
    app: String,
    /// Whether the permission was granted. `None` when the entry stores an
    /// opaque token rather than a "yes"/"no" value, in which case its
    /// presence means the grant is active.
    allowed: Option<bool>,
    name: Option<String>,
    icon: Option<String>,
}
...
async fn fetch_grants() -> zbus::Result<Vec<Grant>> {
    ...
    for &table in TABLES {
        ...
        for id in ids {
            // The `devices` table is keyed by device (camera, microphone)
            // skip any device id this page does not surface.
            if table == "devices" && !DEVICES.contains(&id.as_str()) {
                continue;
            }
            ...
            for (app, values) in permissions {
                // Tables record the choice as a "yes"/"no" permission value.
                // Anything else is an opaque token whose mere presence means
                // the grant is active, so no status is reported for it.
                let allowed = if values.iter().any(|value| value == "yes") {
                    Some(true)
...
/// A displayed category: a table, optionally narrowed to a single id within it.
/// `id` is `None` for tables shown as a whole and `Some` for per-device tables.
type Category = (&'static str, Option<&'static str>);

/// Categories shown on the page, in display order.
fn categories() -> Vec<Category> {
    let mut categories = vec![("screencast", None), ("remote-desktop", None)];
    categories.extend(DEVICES.iter().map(|&device| ("devices", Some(device))));
    categories
}

fn category_title(table: &str, id: Option<&str>) -> String {
    match (table, id) {
        ("screencast", _) => fl!("app-permissions", "screencast"),
        ("remote-desktop", _) => fl!("app-permissions", "remote-desktop"),
        ("devices", Some("camera")) => fl!("app-permissions", "camera"),
        ("devices", Some("microphone")) => fl!("app-permissions", "microphone"),
        (other, id) => id.unwrap_or(other).to_owned(),
    }
}
...
fn grants() -> Section<crate::pages::Message> {
    ...
    Section::default()
        ...
        .view::<Page>(move |_binder, page, _section| {
            ...
            let mut view = widget::column::with_capacity(categories.len()).spacing(space_xxs);

            if page.loaded && page.grants.is_empty() {
                view = view.push(text::body(fl!("app-permissions", "none")));
            }

            for (table, id) in categories {
                let mut section = settings::section().title(category_title(table, id));
                let mut empty = true;

                for (idx, grant) in
                    page.grants.iter().enumerate().filter(|(_, grant)| {
                        grant.table == table && id.is_none_or(|id| grant.id == id)
                    })
                {
                    empty = false;
                    ...
                    if let Some(allowed) = grant.allowed {
                        let status = if allowed {
                            fl!("app-permissions", "allowed")
                        } else {
                            fl!("app-permissions", "denied")
                        };

                        row = row.push(text::body(status));
                    }
