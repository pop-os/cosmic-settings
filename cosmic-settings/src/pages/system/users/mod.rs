...
impl Default for Page {
    fn default() -> Self {
        Self {
            ...
            dialog: None,
            default_icon: icon::from_path(PathBuf::from("/usr/share/pixmaps/faces/pop-robot.png")),
...
impl page::Page<crate::pages::Message> for Page {
            ...
            Dialog::AddNewUser(user) => {
                ...
                let password_input = widget::container(
                    widget::text_input("", &user.password)
                        ...
                        .on_toggle_edit(|is_editing| {
                            eprintln!("editing password: {is_editing}");
                            Message::None
                        })
                        .on_input(|value| {
                            Message::Dialog(Some(Dialog::AddNewUser(User {
    ...
    fn on_enter(
        ...
        // TODO: cancel on exit or reload.
        cosmic::task::future(async { Self::reload().await }).map(crate::pages::Message::User)
    }
}
        ...
        match message {
            ...
            Message::SelectedProfileImage(uid, image_result) => {
                ...
                return cosmic::task::future(async move {
                    ...
                    match result {
                        Ok(_) => Message::LoadedIcon(uid, icon::from_path(path)),
                        Err(why) => {
        ...
        .view::<Page>(move |_binder, page, section| {
            ...
            page.users
                ...
                .flat_map(|(idx, user)| {
                    ...
                    let text_class = if expanded {
                        cosmic::theme::Text::Accent
                    } else {
                        cosmic::theme::Text::Default
                    };

                    let fullname_text = text::body(&user.full_name).class(text_class);
                    ...
                    let expanded_details = expanded.then(|| {
                        let mut details_list = widget::list_column()
                            ...
                            .add(settings::item_row(vec![
                                ...
                                widget::toggler(user.is_admin)
                                    .on_toggle(Message::SelectedUserSetAdmin)
                                    .into(),
                            ]));
                        ...
                        if page.users.len() > 1 {
                            details_list = details_list.add(settings::item_row(vec![
                                widget::horizontal_space().width(Length::Fill).into(),
                                widget::button::destructive(crate::fl!("remove"))
                    ...
                    let account_details = Some(
                        widget::mouse_area(settings::item_row(vec![
                            widget::row::with_capacity(2)
                                .push(profile_icon)
                                .push(
                                    widget::horizontal_space()
                                        .width(Length::Fixed(theme.space_xxs().into())),
                                )
                                .push(
                                    column::with_capacity(2)
                                        .push(fullname_text)
...
fn add_user_button() -> Section<crate::pages::Message> {
    Section::default().view::<Page>(move |_binder, _page, _section| {
        widget::Column::with_children(vec![
            widget::horizontal_space().width(Length::Fill).into(),
            widget::button::suggested(crate::fl!("add-user"))
...
// async fn passwd_set(username: String, password: String) -> io::Result<()> {
//     let mut child = tokio::process::Command::new("passwd")
//         .stdin(std::process::Stdio::piped())
//         .arg(username)
//         .spawn()?;

//     let mut stdin = child.stdin.take().expect("passwd stdin was not piped");
//     _ = stdin
...
//             format!("passwd failed with exit status: {:?}", exit_status),
//         ));
//     }
