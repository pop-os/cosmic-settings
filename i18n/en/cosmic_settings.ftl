...
mobile = Mobile Broadband
    ...
    .network-type = Preferred network type
    .network-type-unavailable = Network type selection is not available for this modem.
    .network-preferences = Network preferences
    .network-type-applying = Applying…
    .network-type-failed = The modem firmware rejected the selected network type, so it was removed from the list.
    .network-automatic = Automatic (3G, 4G, 5G; prefer 5G)
    .network-4g-5g = 4G / 5G (prefer 5G)
    .network-5g-only = 5G only
    .network-3g-4g = 3G / 4G (prefer 4G)
    .network-4g-only = 4G only
    .network-3g-only = 3G only
    .network-custom = { $allowed } (prefer { $preferred })
    .network-bands = Preferred bands
    .network-bands-unavailable = Band selection is not available for this modem.
    .network-bands-edit = Select
    .network-bands-summary = { $selected } of { $supported } supported bands selected
    .network-bands-description = Select the bands the modem may use. At least one band must remain selected.
    .network-bands-warning = Applying a band lock may briefly disconnect mobile data.
    .network-bands-3g = 3G bands
    .network-bands-4g = 4G LTE bands
    .network-bands-5g = 5G NR bands
    .network-bands-other = Other bands
    .network-bands-apply = Apply
    .network-bands-automatic = Automatic
    .network-bands-applying = Applying…
    .network-bands-failed = The modem did not apply the selected bands. Check that they are supported by the current firmware.
    .network-details = Mobile network details
    .network-details-unavailable = Details are not currently available from this modem.
    .network-operator = Operator
    .network-cell = Serving cell
    .network-cell-unavailable = Serving cell information is not available.
    .network-lte-signal = 4G LTE signal
    .network-5g-signal = 5G NR signal
    .network-aggregation = Carrier aggregation
    .network-aggregation-unavailable = Aggregated bands are not exposed by this modem driver.
    .network-signal-guide = Signal quality guide
    .network-signal-guide-description = RSRP: good ≥ −90 dBm, poor < −100 dBm · RSRQ: good ≥ −15 dB, poor < −20 dB · SINR: good ≥ 13 dB, poor < 0 dB
    .signal-excellent = Excellent
    .signal-good = Good
    .signal-fair = Fair
    .signal-poor = Poor
    .signal-very-poor = Very poor
