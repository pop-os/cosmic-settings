...
battery-notice = Plug into power before you begin.
...
os-update = OS Update & Recovery
    .desc = System upgrades, updates, and recovery options.

os-upgrade = OS Upgrade
os-recovery = OS Recovery
os-updates = System and Application Updates

os-upgrade-permission-denied = Permission Required
...
upgrade-available = Version {$version} is available
upgrade-canceling = Canceling upgrade
upgrade-downloading = Version {$version} is currently downloading
upgrade-from-to = Upgrade from {$current} to {$next} is available
upgrade-preparing = Preparing Upgrade
upgrade-ready = Ready to upgrade to {$version}
